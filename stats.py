...
class StatCollector:
    ...
    def __init__(self, target, time: int):
        self.target = target
        self.time: int = time
        self.last_stats: dict[str, int] = {}
        self.diff_stats: list[dict[str, int]] = []
    ...
    def _collect_stats(self):
        ...
        if not isinstance(self.target.stat, dict):
            return
    ...
    def _collect_timings(self):
        ...
        if not isinstance(self.target.timing, list):
            return

        self.timings.append(list(self.target.timing))
        self.target.timings = []
    ...
    def plot_stats(self,
        ...
        if not valid_keys:
            raise ValueError((
                f"Keys not found in stats: "
                f"{set(keys) - set(valid_keys)}"
            ))
        ...
        for pct_idx, pct in enumerate(percentiles):
            ...
            for interval in self.timings:
                if len(interval) == 0:
                    ...
                    val = float(np.percentile(interval, pct)) / 1_000_000.0
                    y_data.append(val)
            ax.plot(x, y_data, label=f"P{pct}", color=colors[pct_idx],
                    linewidth=2)
